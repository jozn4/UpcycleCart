...
if (isset($_GET["request_id"])) {
    ...
    //Get request details with subcategory (to calculate earned coin)
    ...
    if ($display) {
        ...
        $c        = $display['coin'];
        $earned   = $quantity * $c; // total coins earned for this request

        //Update request status + coin field
        ...
        if ($sql1res) {
            //Update wallet balance (only update, no insert)
